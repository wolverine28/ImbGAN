...
from networks.MNIST_networks import (Discriminator_MNIST, Generator_MNIST,
                                     LeNet, latant_mapping, trainSubGAN)
from utils import MyDataset, compute_BA, make_Imb, weights_init
...
parser.add_argument('--beta', type=str, default='0,1', help='Overlapped classes')
parser.add_argument('--pre_train_path', type=str, default=True, help='path of pre-trained GAN')
parser.add_argument('--manualSeed', type=int, default=623, help='seed')
...
def eval(netC, dataloader_test, epoch):
    ...
    with torch.no_grad():
        for data,target in dataloader_test:
            ...
            BA = compute_BA(predicted.view(-1),target)
            test_err.append(BA.item())

    print('             epoch:{:04d}  test_BA:{:.3f}'.format(epoch,np.mean(test_err)))

def train_ImbSubGAN(args,imb_dataset,dataloader, dataloader_test):
    try:
    ...
    netM = latant_mapping(args.nz).cuda()

    # Define the discriminator and initialize the weights
    netSubD = Discriminator_MNIST()
    netSubD.load_state_dict(torch.load(os.path.join(args.path,f'{name}_D.ckpt')))
    netSubD.cuda()

    for ct, child in enumerate(netSubD.children()):
        print(ct,child)
        if ct<2:
    ...
    # setup optimizer
    optimizerSubD = optim.Adam(netSubD.parameters(), lr=args.lr, betas=(0, 0.9))
    optimizerM = optim.Adam(netM.parameters(), lr=args.lr, betas=(0, 0.9))
    ...
    for e in range(args.nepoch):
        ...
        netG, netD = trainSubGAN(args, netG, netSubD, netM, dataloader, optimizerSubD, optimizerM, args.niter, False, 'cuda:0')
        
        z = Variable(FloatTensor(np.random.normal(0, 1, (10000, 128))))
        m_prime_miss = Variable(netG(z).data)
        netG.load_state_dict(torch.load(torch.load(os.path.join(args.path,f'{name}_G.ckpt'))))
        netD.load_state_dict(torch.load(torch.load(os.path.join(args.path,f'{name}_D.ckpt'))))
...
if __name__=='__main__':
    ...
    # train ImbGAN
    train_ImbSubGAN(args,dataset,dataloader, dataloader_test)